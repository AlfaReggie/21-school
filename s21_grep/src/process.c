...
void execute_line(t_flag *flags, regex_t *regex, const char* filename, const char* next_filename)
{
    int c = 0;
    t_getline line = {0};
...
void process_files(t_flag *flags, t_grep information)
{
    ...
    if (regcomp(&regex, information.templates, REG_EXTENDED | REG_NEWLINE | i_case))
        perror("Error");
    if (!information.num_filenames)
        ...
        for (size_t i = 0; i < information.num_filenames; i++)
        {
            if (i == information.num_filenames - 1)
                ...
                execute_line(flags, &regex, information.filenames[i], information.filenames[i+1]);
        }
    regfree(&regex);
}
